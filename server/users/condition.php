...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $errors = condition_validate($condition_date, $weight, $temperature, $defecation, $event, $meal);

    if (
        empty($errors) &&
        insert_condition($condition_date, $weight, $temperature, $defecation, $event, $meal)
    ) {
...
<body>
    <div class="condition_main">
        ...
        <div class="condition_content">
            ...
            <? var_dump($condition_date, $defecation) ?>
            <form class="condition_form" action="" method="post">
                <label class="cdlabel" for="condition_date">Date</label>
                <input type="date" name="condition_dat" id="condition_date" min="2022-11-01" max="2040-12-31" value="<?= h($condition_date) ?>">
                ...
                <input type="radio" neme="defecation" value="1" <?php if ($defecation == 1) echo  "checked" ?>>Yes
                ...
                <input type="checkbox" class="meal" id="meal" name="check[]" value="0" <?php if ($meal == 1) echo  "checked" ?>>Breakfast
                <input type="checkbox" class="meal" id="meal" name="check[]" value="1" <?php if ($meal == 2) echo  "checked" ?>>Lunch
                <input type="checkbox" class="meal" id="meal" name="check[]" value="2" <?php if ($meal == 3) echo  "checked" ?>>Dinner
                <input type="checkbox" class="meal" id="meal" name="check[]" value="3" <?php if ($meal == 4) echo  "checked" ?>>Not eating
